...
def PointPprocessing(hands_Pose, hands_LR):  # 分別處理左右手座標之副程式	(左手要做什麼，右手要做什麼 分別計算)
	global frame, color, dots, take,mod,colorx,colory,colorz
			...
			####################################################################################################################
				...
				if finger_points:
					...
					if text == '2':
						...
						take = True
			if mod==2:
				mainu1 = np.full((10, 10, 3), (0, 0, 0), np.uint8)  # 產生10x10黑色的圖
				mainu1= cv2.resize(mainu1, (int(frame.shape[1]+120), frame.shape[0]), interpolation=cv2.INTER_AREA)
				cv2.rectangle(mainu1, (70, 70), (520, 120), (0, 0, 255, 255), 5)  # 在畫面上方放入紅色正方形
				cv2.rectangle(mainu1, (70, 150), (520, 200), (0, 255, 0, 255), 5)  # 在畫面上方放入綠色正方形
				cv2.rectangle(mainu1, (70, 230), (520,280), (255, 0, 0, 255), 5)  # 在畫面上方放入藍色正方形
				mainu1 = cv2.circle(mainu1,(Pose1[0],Pose1[1]), 10, (255, 255, 255), -1)
				for i in Pose.landmark:
					x = i.x * frame.shape[1]
					y = i.y * frame.shape[0]
					finger_points.append((x, y))
				if finger_points:
					finger_angle = hand_angle(finger_points)  # 計算手指角度，回傳長度為 5 的串列
					text = hand_pos(finger_angle)  # 取得手勢所回傳的內容
					if text=='1' and 70<=int(Pose1[0])<=540 and 70<=int(Pose1[1])<=120:
						colorx=int((Pose1[0]-70)/2)
					elif text=='1' and 70<=int(Pose1[0])<=540 and 150<=int(Pose1[1])<=200:
						colory=int((Pose1[0]-70)/2)
					elif text=='1' and 70<=int(Pose1[0])<=540 and 230<=int(Pose1[1])<=280:
						colorz=int((Pose1[0]-70)/2)
					elif text=='5':
						color=(colorx,colory,colorz)
						mod=1
				cv2.putText(mainu1, str(int(colorx)), (540, 95), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 1, cv2.LINE_AA)
				cv2.putText(mainu1, str(int(colory)), (540, 175), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 1, cv2.LINE_AA)
				cv2.putText(mainu1, str(int(colorz)), (540, 255), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 1, cv2.LINE_AA)
				cv2.rectangle(mainu1, (70, 400), (520, 420), (colorx,colory,colorz),-1)  # 在畫面上方放入紅色正方形
				cv2.rectangle(mainu1, (70, 420), (90, 460), (0,0,255),-1)  # 在畫面上方放入紅色正方形
				cv2.imshow("mainu", mainu1)
	return Pose2
