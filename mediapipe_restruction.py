...
hands = mpHand.Hands(model_complexity=0, max_num_hands=2, min_detection_confidence=0.3,min_tracking_confidence=0.9)  # 001
...
def graphics_menu():
    ...
    cv2.rectangle(graphics_menu, (10, 130), (40, 160), (0, 0, 255), -1)
    graphics_menu = addText(graphics_menu, "橡皮擦", (50, 135), (255, 255, 0), 20)
...
def PointPprocessing(hands_Pose, hands_LR, menu, Main_hand, colormain):  # 分別處理左右手座標之副程式	(左手要做什麼，右手要做什麼 分別計算)
    ...
    if Main_hand == 'Right':
        ...
        pass
    smailblack1 = addText(smailblack1, "主手({})手勢: {}".format(mainhand, main_hand_text),
                          (smailblack1.shape[1] - 145, smailblack1.shape[0] - 60), (255, 255, 0), 15)  # 通道rgb
    smailblack1 = addText(smailblack1, "副手手勢: {}".format(sub_hand_text),
                          (smailblack1.shape[1] - 100, smailblack1.shape[0] - 40), (255, 255, 0), 15)  # 通道rgb
    smailblack1 = addText(smailblack1, "雙手張開，清除畫面", (smailblack1.shape[1] - 145, smailblack1.shape[0] - 20),
                          (255, 0, 0), 15)  # 通道rgb
...
def Function_Select(main_hand_text, sub_hand_text, main_finger_points, sub_finger_points, main_Pose, sub_Pose,
    ...
    if Mode == 'Draw' and sub_hand_text == '8':
        mod = '4'
        Mode = 'Draw'
    if Mode == 'Draw' and sub_hand_text != '8' and mod != '4' and mod != 'graphics':
        ...
        smailblack1 = addText(smailblack1, "功能:畫面調整(主手比'五'退出，回到功能板)", (30, 30), (0, 255, 255),
                              20)  # 使用者文字
        smailblack1 = addText(smailblack1, "請將副手維持一個環形{}秒".format(time_standard_long), (30, 60),
                              (0, 255, 255), 20)  # 使用者文字
    elif Mode == 'zoon_move' and mod == 4:
    ...
    elif mod == 'graphics' and Mode != 'square' and Mode != 'round' and Mode != "Draw":
    ...
    elif mod == 'graphics' and Mode == "Draw":
        smailblack1 = addText(smailblack1, "主手比五退出模式", (30, 30), (87, 205, 55), 20)  # 使用者文字
    else:
        pass
    ...
    if sub_hand_text == '1' and mod == 1 and Mode != 'zoon_move' and Mode != 'graphics' or voice_check_func == "menu":
    ...
    if Mode == 'zoon_move':
        ...
        if 10 <= fy <= 40 and 10 <= fx <= 40 and sub_hand_text == '1' and Mode == 'Func':
            ...
            try:
                ...
                pass
        elif 130 <= fy <= 160 and 10 <= fx <= 40 and sub_hand_text == '1' and Mode == 'Func':
            color = 0
            Mode = 'Draw'
            try:
                cv2.destroyWindow("menu")
        ...
        if main_hand_text == "1" and sub_hand_text == "1" and Mode == 'round':
            ...
            cv2.circle(newblack, main, distance, (colorz, colory, colorx), -1)
            main_Pose2 = []
            text = '1'
    elif Mode == "Draw" and mod == '4':
        key = cv2.waitKey(1)
        ...
        child_black, Mode, smailblack1 = child_Mode(child_black, cover_pics, smailblack1)  # 呼叫child Mode
        if sub_hand_text == '2':
            mod = 1
